---
title: "SBF Exam: Everything You Need to Know"
description: "Complete guide to the German boating license exam: exam structure, pass thresholds, required documents, costs, and how to register correctly."
date: 2026-04-08
updated: 2026-07-21
tags: ["Exam Knowledge"]
readingTime: 7
lang: en
author: "Marius Gehler"
altSlug: "sbf-pruefung-alles-wissenswerte"
faq:
  - question: "From what engine power do I need a boating license?"
    answer: "On German federal waterways, a license is required for boats with more than 11.03 kW (15 HP) engine output on combustion engines; for electric motors the threshold is 7.5 kW. The minimum age to independently operate a motorboat on inland waterways is 16 years."
  - question: "What parts does the SBF exam consist of?"
    answer: "Three parts, all of which must be passed: the theory exam (30 multiple-choice questions in 45 minutes from the official ELWIS catalog), the knot exam, and the practical exam on the water."
  - question: "How many questions do I need to answer correctly in the SBF theory exam?"
    answer: "The 30 questions split into 7 base questions (at least 5 correct) and 23 specific questions (at least 18 correct). Both blocks must reach their minimum score separately, a strong score in one cannot make up for a weak score in the other."
  - question: "What does the SBF cost?"
    answer: "The 2026 exam fee is about €130.78 (SBF Inland), €148.38 (SBF Coastal), or €178.61 for the combined exam. Including a course, total costs are often €400-430 for SBF Inland and around €520 for SBF Coastal, depending on the boating school and region."
  - question: "How long does preparation for the SBF take?"
    answer: "For SBF Inland, studying 30-45 minutes daily, 3-6 weeks is realistic. For SBF Coastal, plan extra time because of the added navigation task. The knot exam requires daily hands-on practice."
  - question: "What happens if I fail a part of the exam?"
    answer: "You only need to retake the part you failed, not the entire exam. A retake is possible at the earliest the next day, but requires a new registration and a separate fee with the examination board. There is no limit on the number of attempts."
---

The German Sportbootführerschein (SBF) is the official license for operating motorboats in Germany. This guide covers everything you need to know before your exam – with accurate data from official ELWIS and DMYV sources.

## When do you need a boating license?

On German federal waterways, a license is **required** for boats with more than **11.03 kW (15 HP)** engine output (combustion engines) or **7.5 kW** (electric motors). Smaller boats don't require a license, but operators must still follow all traffic rules.

The **minimum age** to independently operate a motorboat on German inland waterways is **16 years**.

## The three parts of the SBF exam

All three parts must be passed:

### 1. Theory exam

**Format:** Multiple choice, 30 questions, 45 minutes  
**Source:** Official ELWIS question catalog (published by Generaldirektion Wasserstraßen und Schifffahrt)

The 30 questions are split into two blocks that must **each** be passed separately:

| Block | Questions | Minimum score |
|---|---|---|
| Base questions (general fundamentals) | 7 | at least **5 points** |
| Specific inland questions | 23 | at least **18 points** |
| **Total** | **30** | **at least 23 points** |

> **Critical:** Both blocks must reach their individual minimum. A strong score in one block cannot compensate for a weak score in the other.

**Topics covered:**
- **Navigation** – waterway rules, signs, light signals, sound signals
- **Maritime law** – collision avoidance rules, right of way
- **Seamanship** – maneuvers, anchoring, safety equipment
- **Weather** – weather patterns, storm signals, risk situations
- **Engine knowledge** – propulsion, fuel types, basic maintenance
- **Safety** – [distress signals](/en/blog/distress-signals-at-sea/), rescue equipment, emergency procedures

### 2. Knot exam

You must correctly demonstrate several [sailor's knots](/en/blog/most-important-knots-sbf/). The most important:
- **Bowline (Palstek)** – creates a fixed non-slipping loop
- **Clove hitch (Webleinstek)** – two hitches for securing a line to a post, piling, or bollard
- **Figure-eight knot (Achtknoten)** – stopper knot
- **Square knot (Kreuzknoten)** – joins two lines of equal thickness

The knot exam is a **standalone exam section** – not just a formality. Knots are the most common failure point in the practical SBF exam.

### 3. Practical exam (on the water)

You must independently operate a boat and demonstrate several maneuvers:
- Docking and undocking (at a jetty, buoy, or in a berth)
- Course keeping and steering
- **Man-overboard drill** (recovering a floating object)
- Anchoring and weighing anchor
- Reversing

The practical exam usually takes place on your boating school's training boat, often in a group session with other candidates. While one candidate takes the helm, another is tested on knots in parallel. Per person, the practical part typically takes anywhere from a few minutes to about half an hour, depending on how many maneuvers the examiner asks for.

## What documents do you need to register?

The examination board requires more than just the registration form:

- **Valid ID document or passport**
- **Recent passport photo** in 35 x 45 mm format, no headwear, with name and date of birth written on the back
- **Medical fitness certificate** on the official form, including a vision and hearing test. It's issued by a doctor and stays valid for **12 months** from issue date. Certificates for pilots, divers, or professional drivers aren't accepted

## How to register

1. Find the DMYV exam committee for your region (list at dmyv.de)
2. Complete the registration form and submit the documents listed above
3. Pay the exam fee – it must generally arrive **at least 7 days before the exam**, with the correct payment reference (name, date of birth, exam date). Some committees set longer deadlines, so check directly if unsure
4. Bring on exam day: **ID document** and **exam admission notice**

**Critical:** Forgotten documents or an expired fitness certificate can result in being unable to take the exam, or the exam being marked as failed.

## What does it cost?

| Item | SBF Inland | SBF Coastal |
|---|---|---|
| Exam fee (DMYV) | €130.78 | €148.38 |
| Course fee (typical) | ~€265–299 | ~€370 |
| **Total** | **~€400–430** | **~€520** |

*(As of 07/2026 – exam fees apply nationwide across DMYV/DSV committees, course fees vary by boating school and region. For a full breakdown and money-saving tips, see our [SBF cost overview](/en/sbf-costs/).)*

**Money-saving tip:** If you want both licenses, the **combined exam** costs only €178.61 in exam fees instead of €279.16 separately (€130.78 + €148.38), over €100 in savings.

## How long does preparation take?

For [SBF Inland](/en/sbf-inland/), studying 30–45 minutes daily, **3–6 weeks** is realistic for most candidates. Candidates without prior nautical knowledge need extra time for the light signals and right-of-way rules categories. If you're aiming for [SBF Coastal](/en/sbf-coastal/), plan extra time for the added navigation task. Knot practice requires daily physical repetition – no app can replace tying actual knots.

## What happens if you fail?

No need to panic: if you fail one part of the exam, you only need to retake that part, not the entire exam. A retake is possible at the earliest the next day, but you'll need to register again with the examination board and pay a separate, usually lower, fee for the repeated part. There's no limit on the number of attempts, though a part you've already passed only stays valid for a limited time, your examination board sets the exact deadline. For a detailed walkthrough of the retake process and how to prepare more targeted the second time, see [Failed the SBF Inland Exam? Here's What to Do Next](/en/blog/failed-sbf-inland-exam-what-to-do-next/).

## Summary

The [SBF exam](/en/sbf-exam/) is very achievable – if you know the structure. The most common failure reason: not knowing that the two theory blocks must each be passed separately. For more common questions about the German boating license, check our [FAQ](/en/faq/). Start with the Boatpass app: 15 questions daily for free, official ELWIS catalog, full exam simulation included, so you stay confident right up to exam day.
